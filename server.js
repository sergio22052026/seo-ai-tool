...
app.post('/api/suggest', async (req, res) => {
  ...
  const { company, website, city, kwCount, bizType, rilevanza, visibilita } = req.body;
...
SEDE DELL'AZIENDA: ${cityName} — TUTTE le keyword e i competitor devono riferirsi ESCLUSIVAMENTE all'area geografica di ${cityName} e dintorni, MAI ad altre citta o regioni non pertinenti.
...
app.post('/api/analyze', async (req, res) => {
  if (!isAuthenticated(req)) return res.status(401).json({ error: 'Non autenticato' });
  const { company, website, city, bizType, rilevanza, visibilita, selectedKeywords, selectedCompetitors, companyInfo } = req.body;
  if (!company) return res.status(400).json({ error: 'company mancante' });
  if (!ANTHROPIC_API_KEY) return res.status(500).json({ error: 'API key Anthropic non configurata.' });
  ...
  const cityRef = city || companyInfo?.location || company;
  const prompt = `Sei un esperto SEO e digital marketing. Analizza il brand: "${company}" (sito: ${website}).
SEDE DELL'AZIENDA: ${cityRef} — usa ESCLUSIVAMENTE aree geografiche pertinenti a ${cityRef}, mai citta o regioni non correlate.
