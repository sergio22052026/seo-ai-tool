...
app.post('/api/suggest', async (req, res) => {
  ...
  const geoInstruction = geoAreas && geoAreas.length > 0
    ? `Le keyword DEVONO includere i nomi specifici di questi comuni/aree selezionati: ${geoAreasStr}. Esempio: per il settore infissi con comuni [Jesi, Fabriano, Chiaravalle], genera keyword come "infissi Jesi", "finestre Fabriano", "porte blindate Chiaravalle". Distribuisci le keyword tra i vari comuni selezionati.`
    : `Le keyword devono riferirsi all'area di ${cityName} e dintorni.`;
...
COMPETITOR: Suggerisci competitor reali operanti nella regione ${locationFull.split(',').pop().trim()||'italiana'} e nelle zone limitrofe. Come minimo includi competitor regionali.
